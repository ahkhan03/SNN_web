...
`converged` says the network reached a fixed point. It does not say that fixed point solves your QP. Since v0.5.0 three more fields separate feasibility, optimality diagnostics, and projection termination:

```python
result.joint_feasible             # rows AND bounds feasible together
result.stationarity_residual      # eps-KKT diagnostic at the final point
...
- **`stationarity_residual`** estimates the maximum stationarity, complementarity, and primal defect on a `k0`-scaled active set. Its magnitude depends on the problem scale, so use tolerances meaningful for your QP and compare it across `k0` settings rather than treating one threshold as a universal trust verdict.
